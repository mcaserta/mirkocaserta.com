...
description = "Un esempio di lavoro con dati di serie temporali e CompletableFuture in Java, con calcolo dell'indice di qualità dell'aria e recupero dati concorrente."
[taxonomies]
...
I dati nel mondo reale sono naturalmente molto più caotici di così e potresti
...
Possiamo fare meglio di così? Certo. Usiamo un'arma elegante per tempi più
...
Usiamo una `Queue` perché `ConcurrentLinkedQueue` ci offre una collezione
thread-safe per accumulare valori durante l'elaborazione parallela degli stream.
Ne forniamo l'implementazione usando il metodo `supplier`:
...
In questo caso, l'implementazione è una `ConcurrentLinkedQueue` che ci garantisce
inserimenti thread-safe durante l'elaborazione parallela.
...
![reading memes on github](/images/posts/reading-memes-on-github.png)
...
[^time-series-data]:
    ...
    sequenza di dati indicizzati in ordine di tempo. Time-stamped sono dati
    collezionati in diversi momenti. Questi dati consistono tipicamente in
    misure successive fatta dalla stessa fonte in un intervallo di tempo e sono
    usati per tracciarne il cambiamento nel tempo.

[^creep]:
    Mi piace pensare a questo movimento come a una specie di danza, e la trovo
    sexy. Penso che [I'm a creep, I'm a weirdo](https://youtu.be/XFkzRNyygfk).
