...
description = "An example of working with time series data and CompletableFuture in Java, covering air quality index calculation with concurrent data retrieval."
[taxonomies]
...
This technique is known as _last-value carry-forward_ (sometimes called an
_as-of join_): for each new data point, we look back and pick the most recent
value of each type seen so far. As we advance through the timeline, we always
pair the current reading with the latest available value of the other
type[^creep].
...
We use a `Queue` here because `ConcurrentLinkedQueue` gives us a thread-safe
collection for accumulating values during parallel stream processing. We provide
the implementation using the supplier method:
...
In this case, the implementation is a `ConcurrentLinkedQueue` which gives us
thread-safe insertions during parallel processing.
